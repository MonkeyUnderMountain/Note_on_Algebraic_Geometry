...
\subsection{Cohen-Macaulay schemes}
    ...
    \begin{definition}\label{def:locally_complete_intersection}
        A scheme \(X\) is called a \emph{locally complete intersection} if for every point \(x \in X\), there exists an open neighborhood \(U\) of \(x\) such that \(U\) is isomorphic to a closed subscheme of a smooth scheme \(Y\) over a field, defined by a regular sequence.
