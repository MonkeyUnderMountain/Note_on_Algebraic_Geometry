...
\subsection{The sheaves of differentials}
    ...
    \begin{example}\label{eg:differentials_sheaves_on_projective_space}
        
    \end{example}


    \begin{proposition}\label{prop:Omega_X_is_locally_free_when_X_is_smooth_variety}
        Let \(X\) be a smooth variety over \(\kkk\) of dimension \(n\).
    ...
    \begin{theorem}[Euler sequence for projective bundle]\label{thm:Euler_sequence_for_projective_bundle}
        ...
        Then there is a natural exact sequence of \(\calO_{\bbP_X(\calE)}\)-modules
        \[
...
\subsection{Fundamental sequence}
    ...
    \begin{theorem}[The second fundamental sequence of differentials]\label{thm:the_second_fundamental_sequence_of_differentials}
        ...
        \]
        Suppose further that \(Z\) is a local complete intersection in \(X\).
        Then the above sequence is also exact on the left.
    ...
    \begin{corollary}[Adjunction formula]\label{thm:adjunction_formula_smooth_case}
        Let \(X\) be a smooth variety and \(Z \subseteq X\) be a smooth subvariety of codimension \(1\).
        Then 
        \[ K_Z = (K_X + Z)|_Z. \]
    \end{corollary}
    \begin{proof}
        Since both \(X\) and \(Z\) are smooth, \(Z\) is a local complete intersection in \(X\).
        ...
        Taking the top exterior power, we obtain
