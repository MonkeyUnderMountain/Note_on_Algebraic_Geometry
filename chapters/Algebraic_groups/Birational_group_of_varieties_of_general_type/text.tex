\section{Application: birational group of varieties of general type}
    ...
    \begin{proof}
        ...
        % Consider the rational map defined by the complete linear system associated to \(mK_X\):
        % \[ \psi: X \dashrightarrow \bbP(H^0(X,mK_X)), \quad x \mapsto [s_0(x):s_1(x):\cdots:s_N(x)], \]
        % where \(\{s_0,s_1,\ldots,s_N\}\) is a basis of \(H^0(X,mK_X)\).
        % For \(g \in \Bir(X)\), we have a commutative diagram
        % \[ \begin{tikzcd}
        %     X \arrow[r, dashed,"\psi'"] \arrow[d, dashed, "g"'] & \bbP(H^0(X,mK_X))  \\
        %     X \arrow[r, dashed,"\psi"] & \bbP(H^0(X,mK_X)), \arrow[u,"\overline{g^*}"']
        % \end{tikzcd} 
        % \begin{tikzcd}
        %     x \arrow[mapsto, r] \arrow[mapsto, d] & {[g^*s_0(x):g^*s_1(x):\cdots:g^*s_N(x)]} \\
        %     g(x) \arrow[mapsto, r] & {[s_0(g(x)):s_1(g(x)):\cdots:s_N(g(x))],} \arrow[u,mapsto]
        % \end{tikzcd}
        % \]
        % where \(\overline{g^*}\) is the automorphism of \(\bbP(H^0(X,mK_X))\) induced by the linear automorphism \(g^* \in \GL(H^0(X,mK_X))\) and \(\psi'\) given by the composition.
