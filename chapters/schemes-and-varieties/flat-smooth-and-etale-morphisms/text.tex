% !TeX root = flat-smooth-and-etale-morphisms.tex
\subsection{Flat families}
    ...
    The notation and terminology of flatness can be extended to sheaves of modules over schemes.
    ...
    \begin{proposition}\label{prop:flat_over_curve_iff_torsion_free}
        ...
        Then \(\calF\) is flat over \(X\) iff it is torsion-free.
        \Yang{To be checked.}
    \end{proposition}
    ...
    \begin{theorem}\label{thm:flat_iff_has_the_same_Hilbert_polynomials}
        Let \(X\) be a projective scheme with relatively ample line bundle \(\calO_X(1)\) over a noetherian scheme \(T\).
        Let \(\calF\) be a coherent sheaf on \(X\).
        Suppose that \(\calF\) is flat over \(T\).
        Then the Hilbert polynomials \(P_{X_t,\calF_t}(m)\) are independent of \(t \in T\).
        Conversely, suppose that \(T\) is reduced, the constant Hilbert polynomial \(P_{X_t,\calF_t}(m)\) implies that \(\calF\) is flat over \(T\).
        \Yang{To be checked.}
    \end{theorem}

    \begin{theorem}\label{thm:generic_flatness}
        Let \(S\) be a integral noetherian scheme, \(f:X\to S\) be a morphism of finite type and \(\calF\) be a coherent sheaf on \(X\).
        Then there exists a non-empty open subset \(U \subseteq S\) such that the restriction \(\calF|_{f^{-1}(U)}\) is flat over \(U\).
    \end{theorem}
    \begin{proof}
        \Yang{To be added.}
    \end{proof}
...
\subsection{Base change and semicontinuity}
    ...
    Let \(Y\) be an integral noetherian scheme, \(f:X\to Y\) be a projective morphism, and \(\calF\) be a coherent sheaf on \(X\) which is flat over \(Y\). 
    There is a natural base change map for every point \(y \in Y\) and integer \(i \geq 0\):
    \[
        \varphi^i_y:(\sfR^i f_* \calF)_y \otimes_{\calO_{Y,y}} k(y) \to H^i(X_y,\calF_y).
    \]
    ...
    \begin{theorem}[Semicontinuity of cohomology]\label{thm:semicontinuity_of_cohomology}
        The function \(h^i\) is upper semicontinuous on \(Y\).
    \end{theorem}
    \Yang{To be checked.}


    \begin{theorem}[Grauert's theorem]\label{thm:Grauert_theorem}
        Suppose that for an integer \(i\geq 0\), the function \(h^i(y)\) is constant for all points \(y \in Y\).
        Then the sheaf \(\sfR^i f_* \calF\) is locally free on \(Y\), 
        and for every point \(y \in Y\), the base change map \(\varphi^i_y\) is an isomorphism.
    \end{theorem}

    \begin{theorem}[Cohomology and base change]\label{thm:cohomology_and_base_change}
        Suppose that for a point \(y \in Y\) and an integer \(i \geq 0\), the base change map \(\varphi^i_y\) is surjective.
        Then there exists an open neighborhood \(U\) of \(y\) in \(Y\) such that for every point \(y' \in U\), the base change map is an isomorphism. 
        Moreover, the following are equivalent:
        \begin{enumerate}
            \item the base change map \(\varphi^i_y\) is surjective;
            \item the sheaf \(\sfR^i f_* \calF\) is locally free on an open neighborhood of \(y\).
        \end{enumerate}
        \Yang{To be checked.}
    \end{theorem}
...
\subsection{\'Etale morphisms}
    ...
    \begin{theorem}\label{thm:purity_of_branch_locus}
        Let \(f:X\to Y\) be a morphism of finite type between noetherian integral regular schemes.
        Suppose that \(f\) is \'etale in codimension \(1\), i.e., there exists a closed subset \(Z \subseteq X\) of codimension at least \(2\) such that \(f|_{X\setminus Z}:X\setminus Z \to Y\) is \'etale.
        Then \(f\) is \'etale.
        \Yang{To be checked.}
    \end{theorem}
