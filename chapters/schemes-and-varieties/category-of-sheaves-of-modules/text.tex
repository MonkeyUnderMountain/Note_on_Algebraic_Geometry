% !TeX root = category-of-sheaves-of-modules.tex
...
\subsection{Sheaves of modules, quasi-coherent and coherent sheaves}
    ...
    \begin{slogan}\label{slogan:categories_of_some_specific_sheaves}
        \[ \Sh_X(\Ab) \supseteq \Mod_{\calO_X} \supseteq \QCoh_X \supseteq \Coh_X. \]
    \end{slogan}

    \begin{definition}\label{def:support_of_coherent_sheaves}
        Let \((X,\calO_X)\) be a ringed space, and let \(\calF\) be a coherent sheaf on \(X\).
        The \emph{support} of \(\calF\) is defined to be the set
        \[
            \Supp(\calF) := \{ x \in X \mid \calF_x \neq 0 \},
        \]
        where \(\calF_x\) is the stalk of \(\calF\) at \(x\).
        \Yang{To be checked.}
    \end{definition}
...
\subsection{As abelian categories}
    ...
    \begin{theorem}\label{thm:sheaves_of_ab_mod_qcoh_coh_are_abelian_categories}
        ...
        % \Yang{To be checked.}
        Let \((X,\calO_X)\) be a ringed space.
        All of \(\Sh_X(\Ab), \Mod(\calO_X), \QCoh_X, \Coh_X\) are abelian categories.
    \end{theorem}
    ...
    \begin{remark}\label{thm:O_X_mod_has}
        The category of sheaves of \(\calO_X\)-modules generally does not have enough projectives.
        \Yang{To be checked.}
    \end{remark}

    \begin{theorem}\label{thm:locally_free_resolution_of_coherent_sheaves}
        Let \(X\) be a noetherian, integral, separated, regular scheme.
        Then every coherent sheaf on \(X\) admits a finite resolution by locally free sheaves.
    \end{theorem}
    \begin{proof}
        \Yang{To be continued.}
    \end{proof}
...
\subsection{Relevant functors}
    ...
    \begin{definition}\label{def:dual_sheaves}
        Let \((X,\calO_X)\) be a ringed space, and let \(\calF\) be a sheaf of \(\calO_X\)-modules.
        The \emph{dual sheaf} \(\calF^\vee\) is defined to be
        \[
            \calF^\vee := \calHom_{\calO_X}(\calF,\calO_X).
        \]
        \Yang{To be continued.}
    \end{definition}
    ...
    \begin{definition}\label{def:functor_of_sections_with_support}
        Let \((X,\calO_X)\) be a ringed space, and let \(Z\subseteq X\) be a closed subset.
        The \emph{functor of sections with support in \(Z\)} is defined as follows:
        for an \(\calO_X\)-module \(\calF\), we define
        \[
            \Gamma_Z(X,\calF) := \{ s \in \Gamma(X,\calF) \mid \Supp(s) \subseteq Z \},
        \]
        where \(\Supp(s)\) is the support of the section \(s\).
        \Yang{To be checked.}
    \end{definition}
...
\subsection{Cohomological theory}
    ...
    \begin{definition}\label{def:sheaf_ext_functor}
        Let \((X,\calO_X)\) be a ringed space, and let \(\calF,\calG\) be sheaves of \(\calO_X\)-modules.
        The \emph{\(i\)-th sheaf Ext functor} \(\calExt^i_{\calO_X}(\calF,\calG)\) is defined as the \(i\)-th right derived functor of the sheaf Hom functor \(\calHom_{\calO_X}(\calF,-):\Mod(\calO_X)\to \Mod(\calO_X)\) applied to \(\calG\), i.e.,
        \[
            \calExt^i_{\calO_X}(\calF,\calG) := \sfR^i \calHom_{\calO_X}(\calF,\calG).
        \]
        \Yang{To be checked.}
    \end{definition}
