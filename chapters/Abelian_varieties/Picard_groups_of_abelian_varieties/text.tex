...
\subsection{Pullback along group operations}
    ...
    \begin{example}\label{eg:describe_Phi_on_elliptic_curves}
        Let \(E\) be an elliptic curve over \(\kkk\) with origin \(0\) and \(\calL = \calO_E(n \cdot 0)\) for some \(n \in \bbZ\).
        Then for any \(x \in E(\kkk)\), we have 
        \[ \Phi_\calL(x) = t_x^*\calO_E(n \cdot 0) \ten \calO_E(-n \cdot 0) \cong \calO_E(n \cdot x - n \cdot 0). \]
