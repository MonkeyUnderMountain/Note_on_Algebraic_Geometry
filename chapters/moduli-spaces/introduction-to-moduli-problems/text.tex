% !TeX root = introduction-to-moduli-problems.tex
\subsection{Moduli problem by representable functors}
    ...
    \begin{remark}\label{rmk:moduli_functor_as_presheaf_and_fibred_category}
        Let \(S\) be a noetherian scheme and \(\calM: \Sch_{S}^{\op} \to \Set\) be a functor.
        In other words, \(\calM\) is a presheaf on the category \(\Sch_{S}\) with values in \(\Set\).
        By \cref{eg:presheaves_as_category_fibered_in_set}, we can view \(\calM\) as a category  fibred in sets over \(\Sch_{S}\).

        For every \(X \in \Sch_{S}\), the fiber category \(\calM(X)\) is the set of ``deformation families over \(X\)''.
        If \(x \in \Sch_{S}\) is a point, then the fiber category \(\calM(x)\) is the isomorphism classes of objects represented by the point \(x\).
    \end{remark}
